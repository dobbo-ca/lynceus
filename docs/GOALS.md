...
| 2 | pganalyze parity | 🟡 **In progress** | M2–M6 epics; ~80 features in `docs/specs/2026-05-29-lynceus-features.md`. **Layer 0 merged** (PR #19: schema/table readers, insight+plan UI, capability gate, log source). **Layer 1 merged** (PR #20: 5 EXPLAIN insights, Index + VACUUM advisors, wait histograms). **Layer 2 merged** (PR #23: Checks/Alerts engine framework + TXID/MultiXact wraparound + Freezing view + 5 more EXPLAIN insights + Index-Advisor bundle). `.22` Connections + `.23` Schema merged (#26/#27); **perf + vacuum checks merged** (PR #35: partition cache, collector fan-out, xmin/frequency checks); **M3 pg_settings** `.24` Settings checks + `.18` config advisor merged (PR #36). Remaining Layer-2 `.21`/`.25`, `ly-32k` remainder + notifications, Layer 3 log breadth + Workstream B (HA/perf) ahead. |
| 3 | Performance review + CI tooling | 🟡 **In progress** | CI lint+bench merged (`.golangci.yml`, `lint.yml`); `ly-3na` CopyFrom writes merged; reader/writer split merged (`ly-ry1`). `ly-bsf` partition cache + `ly-awh` collector fan-out **merged** (PR #35). Remaining: Workstream B (Helm/probes/retention/load-test). |
...
- **2026-07-08 (perf + vacuum checks bundle)** — Branch `perf-vacuum-checks-7k3q` off `origin/main` @ `0f644a9` (#34). Workflow-orchestrated, subagent-driven TDD (scout → sequential implement → verify), 3 beads: **`ly-bsf`** per-process `sync.Map` weekly-partition cache (+ drop eviction) — storage-side only, no wire change; **`ly-awh`** `collector.RunBounded` bounded reader fan-out + global query budget (`LYNCEUS_QUERY_BUDGET`, default 3), `runFull` now fans out all 6 catalog readers, ship-gate preserved; **`ly-32k`** 2 of 3 remaining vacuum checks — `vacuum.insufficient_frequency` (pure check over `table_stats`, autovacuum-trigger model) and `vacuum.xmin_horizon` (full literal-free FreezeAge-template path: new T1 `XminHorizon` proto {age count + fixed `holder_kind` label} → gated cluster-global `XminHorizonReader` → ingest persist → `xmin_horizon` partitioned table (migration `0013`) → scheduler → `XminHorizonCheck`; contract test extended). Check (3) inefficient index-cleanup phase **deferred** (only signal is live `pg_stat_progress_vacuum`, not in `table_stats`) → new follow-up bead **`ly-08j`**. Full suite green on real Postgres: `go test ./... -p 1` → **17 pkgs all ok** (store 93s, e2e 4.9s); collector outbound-only re-verified (0 `pgxpool.New` / 0 store imports in `internal/collector`); T1 contract green. **Merged as PR #35 (`7a67763`).**

- **2026-07-09 (M3 pg_settings bundle)** — Branch `pg-settings-checks-4m9x`, originally cut off `0f644a9` (#34), **rebased onto `origin/main` @ `7a67763` (#35)** to resolve a three-way collision with the perf+vacuum bundle. Workflow-orchestrated, subagent-driven TDD (scout → 3 sequential implementers → verify **+ a dedicated adversarial-privacy pass**). Shared T1 foundation: gated collector `SettingsReader` shipping a **curated 31-GUC allowlist** of safe-valued tuning settings (`WHERE name = ANY($allowlist)`, never `SELECT *`), new literal-free T1 `Setting` proto message {name,value,unit,source,pending_restart} on **Snapshot field 14** (renumbered from 13 during the rebase — #35 took 13 for `xmin_horizons`), `settings` partitioned store table (**migration `0014`**, renamed from 0013 to clear #35's `0013_xmin_horizon.sql`), ingest routing, `caps.Settings` gate. On top: **`ly-u4t.24`** 5 Settings checks (`settings.disabled_features/fsync/shared_buffers/stats/work_mem`) plugged into the checks engine; **`ly-u4t.18`** `advisor.ConfigAdvice` pure fn + `/config-advisor` page (settings-derivable MVP; workload-correlated advice deferred → `ly-u4t.30`). **Privacy:** adversarial reviewer verdict **SAFE** — every dangerous GUC (`log_line_prefix`, `search_path`, `application_name`, `primary_conninfo`, `ssl_*_file`, `archive_command`, `*_directory`, custom GUCs) confirmed excluded by the allowlist; dual contract tests (proto field-set + collector allowlist/denylist). The `value` proto field is literal-capable by type, so the redaction boundary is deliberately the reader's name allowlist, not the wire. Full suite green on real Postgres: `go test ./... -p 1`; collector outbound-only re-verified; T1 contract green. **Merged as PR #36.**
